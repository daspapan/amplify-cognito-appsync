...
export function createAmplifyHosting(scope: Construct, props: AmplifyHostingProps){
    const amplifyDeployCDKRole = new Role(scope, `${props.appName}-allow-amplify-deploy-cdk-role`, {
        assumedBy: new ServicePrincipal('amplify.amazonaws.com'),
        description: `Role assumed by Amplify Hosting for deploying AWS CDK frontend app`,
        ...
        buildSpec: BuildSpec.fromObjectToYaml({
            ...
            frontend: {
                phases: {
                    preBuild: {
                        commands: [
                            'cd infra', //the buildspec file gets ran from the root of our project
